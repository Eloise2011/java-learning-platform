#!/usr/bin/env python3
"""Generate complete index.html with JSON-serialized curriculum. All string escaping via json.dumps()."""
import json, os, sys
...
def topic(id, phase, pc, pclass, title, hrs, comp, imp, textbook, summary, lesson, quiz, ex=None):
    topics.append(dict(id=id, phase=phase, phaseColor=pc, phaseClass=pclass,
        title=title, hours=hrs, complexity=comp, importance=imp,
        textbook=textbook, summary=summary, lesson=lesson, quiz=quiz, codingExercise=ex))
...
sys.stdout.write("Building curriculum...\n")
